	...
	try {
		...
		stage('SonarQube Analysis'){
			withCredentials([string(credentialsId: 'SonarQubeToken', variable: 'SONARQUBE_TOKEN')]) {
				//sh "${sonarScannerHome}/bin/sonar-scanner -Dsonar.host.url=http://20.62.252.18:9000/ -Dsonar.login=${SONARQUBE_TOKEN} -Dsonar.projectKey=com.example:java-example-project"
			}
		}
